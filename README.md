...
3. 选择音源模式：
   - **内置音源**（推荐）：使用 Songloft 内置洛雪音源，无需额外配置
     - 选择「音源平台」（酷我/酷狗/QQ/网易云/咪咕），导入时从此平台搜索匹配歌曲
   - **外部 API**：填入洛雪音源 API 服务器地址（如 `http://192.168.1.100:8080`），如有密钥验证则填入 API 密钥
     - 选择「搜索来源」用于跨平台匹配
4. 选择「默认音质」（128k/320k/FLAC/FLAC 24bit）
5. 点击「测试连接」确认音源可用
6. 点击「保存设置」
...
最近被裁员了。如果你喜欢这个项目，或者它刚好帮到了你，欢迎请我喝一杯咖啡。感谢你的支持，也感谢每一个 Star、Issue 和建议。

<p align="center">
  <img src="wechat-pay.jpg" alt="微信支付收款码" width="360">
  <img src="alipay.jpg" alt="支付宝收款码" width="360">
</p>
